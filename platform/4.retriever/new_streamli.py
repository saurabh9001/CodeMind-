                ...
                with tab2:
                    ...
                    for r in results:
                        ...
                        with st.expander(f"**#{r['rank']} {chunk['id']}** · Distance: {r['distance']:.4f}", expanded=(r['rank'] <= 2)):
                            ...
                            source_code = None
                            # Try both 'source_code' and 'source' (dict with 'source_code')
                            if 'source_code' in chunk:
                                source_code = chunk['source_code']
                            elif 'source' in chunk and isinstance(chunk['source'], dict):
                                source_code = chunk['source'].get('source_code')
                            if source_code:
                                st.markdown("**<span style='color:#764ba2'>Source Code:</span>**", unsafe_allow_html=True)
                                st.code(source_code, language="java")
